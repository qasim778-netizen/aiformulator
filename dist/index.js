...
Create a detailed DALL-E prompt that specifically incorporates these visual elements from the reference image while maintaining the required text layout. Be very specific about colors, shapes, patterns, and design elements you see in the reference. Return only the DALL-E prompt.`
    ...
    try {
      ...
      const imagesDir = path7.join(process.cwd(), "client", "public", "images", "generated");
      await fs6.mkdir(imagesDir, { recursive: true });
      const filePath = path7.join(imagesDir, fileName);
      await fs6.writeFile(filePath, Buffer.from(imageBuffer));
...
var LOCAL_UPLOAD_ROOT = path3.resolve(
  process.env.UPLOADS_DIR || path3.join(process.cwd(), "uploads")
);
var localImageUpload = multer({
...
function safeFilename(filename) {
  const extension = path3.extname(filename).toLowerCase() || ".jpg";
  const base = path3.basename(filename, path3.extname(filename)).toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 100) || "image";
  const allowedExtensions = /* @__PURE__ */ new Set([".jpg", ".jpeg", ".png", ".webp", ".gif"]);
  return {
    base,
    extension: allowedExtensions.has(extension) ? extension : ".jpg"
  };
}
async function saveLocalImage(file, folderInput) {
  const folder = safeFolder(folderInput);
  const { base, extension } = safeFilename(file.originalname);
  const suffix = crypto3.randomBytes(4).toString("hex");
  const filename = `${base}-${suffix}${extension}`;
  const directory = path3.join(LOCAL_UPLOAD_ROOT, folder);
  const filePath = path3.join(directory, filename);
  const thumbnailFilename = `${base}-${suffix}-thumb.jpg`;
  const thumbnailPath = path3.join(directory, thumbnailFilename);
  await fs3.mkdir(directory, { recursive: true });
  await fs3.writeFile(filePath, file.buffer);
  try {
    await sharp(file.buffer).resize(400, 300, { fit: "cover", position: "center" }).jpeg({ quality: 80, progressive: true }).toFile(thumbnailPath);
  } catch (error) {
...
async function generateLocalThumbnail(imagePath) {
  if (!imagePath.startsWith("/uploads/")) return null;
  const relativePath = imagePath.slice("/uploads/".length);
  const sourcePath = path3.resolve(LOCAL_UPLOAD_ROOT, relativePath);
  const rootWithSeparator = `${LOCAL_UPLOAD_ROOT}${path3.sep}`;
  ...
  try {
    await sharp(sourcePath).resize(400, 300, { fit: "cover", position: "center" }).jpeg({ quality: 80, progressive: true }).toFile(thumbnailPath);
...
`;
      ...
      xml += url2(`${baseUrl}/contact`, "0.4", "monthly", today);
      xml += url2(`${baseUrl}/terms-of-service`, "0.3", "monthly", today);
      xml += url2(`${baseUrl}/privacy-policy`, "0.3", "monthly", today);
      xml += url2(`${baseUrl}/disclaimer`, "0.3", "monthly", today);
  ...
  const destroySessionAndRedirect = (req, res, isGet) => {
    req.session.destroy((err) => {
      if (err) {
        return isGet ? res.redirect("/?logout=error") : res.status(500).json({ message: "Failed to logout" });
      }
      res.clearCookie("connect.sid");
      return isGet ? res.redirect("/") : res.json({ success: true });
    });
  };
  app2.post("/api/logout", (req, res) => destroySessionAndRedirect(req, res, false));
  app2.get("/api/logout", (req, res) => destroySessionAndRedirect(req, res, true));
  ...
  app2.post("/api/uploads/local", requireAdmin, localImageUpload.single("file"), async (req, res) => {
    try {
      if (!req.file) {
...
Allow: /disclaimer`;
    ...
    try {
      ...
      if (formulation.pdfPath) {
        ...
        if (fs6.existsSync(pdfPath)) {
          pdfBuffer = fs6.readFileSync(pdfPath);
          ...
          try {
            ...
            const filename2 = path7.basename(pdfPath);
            const STORAGE_DIR2 = path7.join(process.cwd(), "formulation_files");
            const fullPath = path7.join(STORAGE_DIR2, filename2);
            if (fs6.existsSync(fullPath)) {
              pdfBuffer = fs6.readFileSync(fullPath);
  ...
  app2.use("/images/generated", express.static(path4.join(process.cwd(), "client/public/images/generated")));
  ...
  if (formulationMatch) {
    ...
    try {
      ...
      if (formulation) {
        ...
        const description = formulation.metaDescription || (nameHasFormulation ? `Professional ${formulation.name} with complete manufacturing guide, ingredient list, and technical specifications.` : `Professional ${formulation.name} formulation with complete manufacturing guide, ingredient list, and technical specifications.`);
        const rawName = formulation.name;
        const nameEndsWithFormula = /\bformula(?:tion)?s?\b$/i.test(rawName.trim());
        const patternTitle = nameEndsWithFormula ? `${rawName} | ${SITE_NAME}` : `${rawName} Formula | ${SITE_NAME}`;
        const baseTitle = formulation.seoTitle && seoTitleIsRelated ? formulation.seoTitle : patternTitle;
        ...
        return {
          ...
          ogImage: formulation.image || void 0,
  ...
  if (categoryMatch) {
    ...
    try {
      ...
      if (category) {
        ...
        const description = category.metaDescription || (catNameHasFormulations ? `Browse professional ${category.name} \u2014 complete ingredient lists, manufacturing processes, and technical specifications for each formula.` : `Browse professional ${category.name.toLowerCase()} formulations. Complete manufacturing guides with ingredients and instructions.`);
        const formulations2 = await storage.getFormulationsByCategory(String(category.id));
  ...
  if (ogUrlValue) {
    if (html.includes('property="og:url"')) {
      html = html.replace(
        ...
        `<meta property="og:url" content="${escapeHtml2(ogUrlValue)}" />`
      );
    } else {
      html = html.replace("</head>", `  <meta property="og:url" content="${escapeHtml2(ogUrlValue)}" />
  </head>`);
    }
  }
  if (meta.ogImage) {
    const absImg = meta.ogImage.startsWith("http") ? meta.ogImage : `${SITE_URL}${meta.ogImage}`;
    const escapedImg = escapeHtml2(absImg);
...
var app = express3();
app.use("/uploads", express3.static(LOCAL_UPLOAD_ROOT, {
  maxAge: "7d",
  immutable: false
}));
app.use(compression({
...
app.use((req, res, next) => {
  ...
  if (path7.startsWith("/formulation/")) {
    const slug = path7.replace("/formulation/", "");
  ...
  try {
    ...
    const serveSeoPage = async (req, res, next) => {
      ...
      if (app.get("env") === "development") {
        htmlPath = path6.resolve(import.meta.dirname, "..", "client", "index.html");
      } else {
        htmlPath = path6.resolve(import.meta.dirname, "public", "index.html");
      }
      let seoMeta;
      try {
      ...
      if (!seoMeta) {
        if (isDynamicRoute) {
          try {
            const html2 = await fs5.promises.readFile(htmlPath, "utf-8");
      ...
      let html;
      try {
        html = await fs5.promises.readFile(htmlPath, "utf-8");
      } catch (e) {
        return next();
      }
